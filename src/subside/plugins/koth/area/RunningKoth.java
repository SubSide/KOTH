		...
		if (cappingPlayer != null) {
			if (++timeCapped < captureTime) {
				if (timeCapped % 30 == 0) {
                    ...
                    if(Bukkit.getPlayer(cappingPlayer) != null){
                        new MessageBuilder(Lang.KOTH_CAPTIME_CAPPER).maxTime(maxRunTime).time(captureTime, timeCapped).player(cappingPlayer).area(area.getName()).buildAndSend(Bukkit.getPlayer(cappingPlayer));
                    }
				}
			} else {
