...
public class CommandHandler implements CommandExecutor {
	...
	public void loot(Player player, String[] args) {
		...
		String ar = "";
		if (args.length > 0) {
			if (args[0].equalsIgnoreCase("set") && player.hasPermission("koth.admin")) {
				if (args.length > 1) {
	...
	public void schedule(Player player, String[] args) {
		if (args.length > 0 && player.hasPermission("koth.admin")) {
			...
			for (Day day : Day.values()) {
				...
				for (Schedule sched : schedules) {
					if (sched.getDay() == day) {
						subList.add(new MessageBuilder(Lang.COMMAND_SCHEDULE_LIST_ENTRY).day(day.getDay()).area(sched.getArea()).time(sched.getTime()).length(sched.getRunTime()).build());
					}
				}
				if (subList.size() > 0) {
	...
	public void help(Player player, String[] args) {
		...
		if (player.hasPermission("koth.admin")) {
			...
			list.add(new MessageBuilder(Lang.COMMAND_HELP_TITLE).build());
			list.add(new MessageBuilder(Lang.COMMAND_HELP_INFO).command("/koth loot").commandInfo("Shows the loot for the upcoming koth").build());
			list.add(new MessageBuilder(Lang.COMMAND_HELP_INFO).command("/koth schedule").commandInfo("Shows the schedule for koths").build());
			list.add(new MessageBuilder(Lang.COMMAND_HELP_INFO).command("/koth info").commandInfo("Shows info about this plugin").build());
		}
