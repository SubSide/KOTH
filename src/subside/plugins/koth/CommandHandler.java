...
public class CommandHandler implements CommandExecutor {
    ...
    public void help2(CommandSender sender, String[] args) {
        ...
        for (String hlp : list) {
            ...
            try {
                mB = mB.area(KothHandler.getRunningKoth().get().getArea().getName()).time(KothHandler.getRunningKoth().get().getTimeObject()).player(KothHandler.getRunningKoth().get().getCappingPlayer());
            } catch(Exception e){}
            list2.add(new MessageBuilder(hlp).build());
        }
    ...
    public void info(CommandSender sender, String[] args) {
        ...
        list.add(new MessageBuilder("&2Version: &a" + KothPlugin.getPlugin().getDescription().getVersion()).build());
